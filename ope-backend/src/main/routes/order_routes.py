...
from src.domain.dto import ConfirmOrder as ConfirmOrderDto
from src.main.compose import create_order_composer
from src.main.compose import list_orders_composer
from src.main.compose import confirm_order_composer
...
confirm_order = order_namespace.model('ConfirmOrder', ConfirmOrderDto)
...
class Orders(Resource):
    ...
    @order_namespace.expect(confirm_order)
    @order_namespace.doc(responses={200: 'OK',
                                   400: 'Bad Request',
                                   409: "Integrity Error",
                                   404: "Not Found",
                                   500: "Internal Server Error"})
    def patch(self):
        response = flask_adapter(request, confirm_order_composer())
        return make_response(jsonify(response), int(response["status"]))
